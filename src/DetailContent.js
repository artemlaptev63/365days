...
class DetailContent extends Component {
  ...
  nextPhoto() {
    ...
    if (
      this.state.numberOfPhoto.toString() ===
      (this.props.imagesLength - 1).toString()
    ) {
  ...
  componentWillMount() {
    ...
    localStorage.setItem(
      this.props.itemId,
      localStorage.getItem(this.props.itemId)
    );
    // изменяем state
    this.setState({
      like: localStorage.getItem(this.props.itemId)
  ...
  componentWillReceiveProps(nextProps) {
    ...
    localStorage.setItem(
      nextProps.itemId,
      localStorage.getItem(nextProps.itemId)
    );
    this.setState({
